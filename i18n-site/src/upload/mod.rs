...
pub type LangUrl = (LangDirName, ExtUrlLi);

pub struct Payload {
  site: Site,
  dir: Box<Path>,
  nav_li: Vec<String>,
  url_li_ext: Vec<UrlLiExt>,
  ext_url_li: Vec<LangUrl>,
}

#[allow(async_fn_in_trait)]
pub trait Upload {
  async fn upload(&mut self, payload: &Payload) -> Result<()> {
    let site = &payload.site;
    let url_li_ext = &payload.url_li_ext[..];
    let ext_url_li = &payload.ext_url_li[..];
    let dir = &payload.dir;

    let api_site = api::Site {
      lang_li: self
        .upload_lang(dir, &site.ver, &payload.nav_li, url_li_ext, ext_url_li)
        .await?,
      host: site.host.clone(),
      nav_li: site.nav_li.clone(),
      route_li: site.route_li.clone(),
      url_li_ext: payload.url_li_ext.clone(),
    };
    self
      .upload_site(
        site.channel.clone(),
...
pub async fn upload(
  ...
  for i in lang_path {
    ...
    'next_file: for entry in WalkDir::new(&dir)
      .into_iter()
