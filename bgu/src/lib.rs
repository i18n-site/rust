...
use ifs::{conf, dir::BIN_HOME, rsync, rsync::WalkDir};
use iget::Down;
...
async fn _bgu(h3: bool, url: String, name: &str, now_ver: &Ver) -> Result<Option<Downing>> {
  let site = {
...
async fn bgu(
  ...
  now_ver: Ver,
  mut li: Vec<(bool, String)>,
) -> Result<Option<Downing>> {
  let name = name.into();
  while let Some((h3, url)) = li.pop() {
    if let Ok(r) = xerr::ok!(_bgu(h3, url, &name, &now_ver).await) {
  ...
  pub fn new(
    ...
    Some(Self {
      ...
      ing: tokio::spawn(bgu(
        ...
        name,
        ver,
        randiter::RandIter::new(li)
          .map(|(h3, url)| (*h3, url.as_ref().to_owned()))
          .collect(),
      )),
      pk,
    })
