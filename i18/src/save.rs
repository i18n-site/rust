...
impl<'a> Save<'a> {
    ...
    for (rel, api::TranedLi { li }) in traned {
      for api::Traned { lang, txt } in li {
        ...
        if let Some(w) = self.waiting.get_mut(rel) {
          ...
          if !w.lang_meta.contains_key(&lang) {
            for (prefix, ft) in &self.rel_ft {
              if rel.starts_with(prefix) {
                update_hash.push((
                  ...
                  Meta {
                    hash: xhash(txt),
                    to_li: if let Some(to_li) = ft.to_li(lang) {
  ...
  pub fn end(self) {
    ...
    if self.has_waiting() {
      ...
      for (rel, w) in self.waiting {
        for lang in w.to_li {
          println!("  {}/{}", LANG_CODE[lang as usize], rel);
        }
      }
    }
  }
