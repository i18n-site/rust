...
#[dynamic]
pub static API_TRAN_END: String = format!("{}/tranEnd", API());
  ...
  if total_len > 0 {
    ...
    for (li, db, ext, from_to) in all_li {
      ...
      for i in li {
        ...
        match tran_path(i, &token, &db, ext, workdir, &from_to).await {
          Err(err) => {
            if let Some(e) = err.downcast_ref::<crate::Err>() {
              if e.is_exit() {
                return Err(err);
    ...
    if char_sum > 0 {
      ...
      let asset = ireq::REQ
        ...
        .await?;
      if asset.status() != 200 {
        eprintln!("❌ {} {}", &*API_TRAN_END, asset.text().await?);
      } else {
        let asset = asset.bytes().await?;
