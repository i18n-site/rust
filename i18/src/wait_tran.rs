...
use crate::{api, fetch_tran, print_err, Save, COST_BASE, COST_BASE_U64};

const BASE: f64 = 100.0;

pub async fn wait_tran<'a>(traning: api::Traning, id: &str, mut save: Save<'a>) -> Result<()> {
  if save.save(&traning.traned)? {
    loop {
