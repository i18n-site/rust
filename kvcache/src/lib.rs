...
use fred::prelude::{FredResult, Value};
use rany::{Rany, B255};
...
pub trait Cache {
  async fn get_li(&self, kind: u64, bin_li: &[&[u8]]) -> FredResult<Vov> {
    let kind = &B255.e(kind);
    self._get_li(kind, bin_li).await
  }

  async fn set_li<V: Send + Into<Value>>(&self, kind: u64, map: Map<'_, V>) -> FredResult<()> {
    let kind = &B255.e(kind);
    self._set_li(kind, map).await
  }

  // suffix should use b255 encode
  async fn _get_li(&self, suffix: &[u8], bin_li: &[&[u8]]) -> FredResult<Vov>;

  async fn _set_li<V: Send + Into<Value>>(&self, suffix: &[u8], map: Map<'_, V>) -> FredResult<()>;
...
pub use user_cache::{prefix_key, UserCache};
