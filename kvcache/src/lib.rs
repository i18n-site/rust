...
use fred::prelude::{RedisResult, RedisValue};
use rany::{Rany, B255};
...
pub trait Cache {
  async fn get_li(&self, kind: u64, bin_li: &[&[u8]]) -> RedisResult<Vov> {
  ...
  async fn _get_li(&self, suffix: &[u8], bin_li: &[&[u8]]) -> RedisResult<Vov>;

  async fn _set_li<'a, V: Send + Into<RedisValue>>(
    &self,
    suffix: &[u8],
    map: Map<'a, V>,
  ) -> RedisResult<()>;
