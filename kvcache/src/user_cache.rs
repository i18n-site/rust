use fred::{
  ...
  prelude::{Client, FredResult, Value},
};
use rany::{Rany, B255};
use xbin::concat;
...
impl<'a> UserCache<'a> {
  ...
  pub fn user_kvli<V: Send + Into<Value>>(
    &self,
    suffix: &[u8],
    map: Map<'_, V>,
  ) -> Vec<(Box<[u8]>, V)> {
  ...
  pub fn global_kvli<V: Send + Into<Value>>(
    &self,
    suffix: &[u8],
    map: Map<'_, V>,
  ) -> Vec<(Box<[u8]>, V)> {
