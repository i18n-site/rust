...
use crate::{
  Api, Response, api,
  api::{AccessToken, IdAccessToken, MsgLi, post},
...
async fn access_token(client_id: &str, client_secret: &str) -> Result<TokenResponse> {
  let url = format!(
    "https://aip.baidubce.com/oauth/2.0/token?client_id={client_id}&client_secret={client_secret}&grant_type=client_credentials"
  );
...
impl Baidu {
  pub fn loads(yml: impl AsRef<[u8]>) -> Result<Self, serde_yaml::Error> {
    ...
    let cache: BaiduConf = confdir().join("aiapi").join("baidu").into();
