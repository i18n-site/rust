...
start_idx=$((RANDOM % V_HOST_LEN))
for ((i = 0; i < V_HOST_LEN; i++)); do
  idx=$(((start_idx + i) % V_HOST_LEN))
  url="https://${V_HOST[$idx]}/v/${APP}"
  eurl $url
  ver=$($CURL -sS $url)
...
start_idx=$((RANDOM % BIN_HOST_LEN))
for ((i = 0; i < BIN_HOST_LEN; i++)); do
  idx=$(((start_idx + i) % BIN_HOST_LEN))
  prefix=${BIN_HOST[$idx]}
