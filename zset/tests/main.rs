...
#[test]
fn test_add_and_card() {
  // Test adding new elements and checking cardinality.
  // 测试添加新元素和检查基数。
  ...
  assert_eq!(zset.get_with_score(0), Some((Arc::new("two"), 2)));
  assert_eq!(zset.get(1).map(|v| *v), Some("one"));
  assert_eq!(zset.get_with_score(1), Some((Arc::new("one"), 10)));
}
...
fn test_range() {
  ...
  let r: Vec<&str> = zset.range(0..zset.card()).iter().map(|v| **v).collect();
...
fn test_concurrent_add_remove() {
  ...
  assert_eq!(zset.card(), 100);

  // Concurrently remove even-numbered elements.
  // 并发删除偶数元素。
...
fn test_float_scores() {
  ...
  // 测试使用浮点数分数的 Zset。
  let zset = Zset::<&str, OrderedFloat<f64>>::new();
    ...
    handles.push(thread::spawn(move || {
      ...
      for _ in 0..num_ops_per_thread {
        ...
        match op {
          ...
          4 => {
            let card = zset_clone.card();
            if card > 0 {
              let start = rng.random_range(0..card);
              let end = rng.random_range(start..=card);
