...
async fn request(client: &Client, token: &str, method: Method, url: &str) -> Result<String> {
...
async fn json<T: serde::de::DeserializeOwned>(
  ...
  let text = request(client, token, method, url).await?;
...
impl User {
  ...
  pub async fn request(&self, method: Method, url: impl AsRef<str>) -> Result<String> {
    request(&self.client, &self.token, method, url.as_ref()).await
  }

  pub async fn json<T: serde::de::DeserializeOwned>(&self, url: impl AsRef<str>) -> Result<T> {
    json(&self.client, &self.token, Method::GET, url.as_ref()).await
  }
