...
pub const TEST_RESPONSE_BODY: &[u8] = b"Hello World";
...
async fn main() -> anyhow::Result<()> {
  ...
  let upstream = Upstream {
    addr_li: vec![UPSTREAM_ADDR.parse()?].into(),
  ...
  let h1_addr: SocketAddr = H1_ADDR.parse()?;
  let h2_addr: SocketAddr = H2_ADDR.parse()?;
